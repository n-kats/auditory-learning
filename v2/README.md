...
`AUDITORY_LEARNING_V2_PROMPT_PATH` が指すファイルの内容を、document ごとの既定プロンプトとして使います。相対パスはリポジトリルート基準で解決します。環境変数は `scripts/launch_v2.sh` と `docker compose` で渡します。backend は `.env` を直接読みません。`AUDITORY_LEARNING_V2_HOST` は外部から渡してください。
...
- `GET /sessions/{request_id}/settings` と `PATCH /sessions/{request_id}/settings` で prompt / model を更新します。
- session の詳細ブロックには prompt / model に加えて、累積生成回数、処理時間、token 数、コストを表示します。
...
backend の Postgres は、論文の正本を `papers`、実行単位を `sessions`、処理結果を `session_results` に分けています。1 つの result は 1 つの paper と 1 つの session に属します。利用情報は `session_usage_records` に append-only で積み、session には token 数とコストの累計を持たせます。
